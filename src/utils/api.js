...
//服务器接口地址
const host = 'http://larabbs.app.com/api'
...
const login = async (params = {}) => {
	...
	if (authResponse.statusCode === 201) {
		...
		wepy.setStorageSync('access_token_expired_ar', new Date().getTime() + authResponse.data.expires_in * 1000)
	}
...
const refreshToken = async (accessToken) => {
	...
	if (refreshResponse.statusCode === 200) {
		...
		wepy.setStorageSync('access_token_expired_ar', new Date().getTime() + refreshResponse.data.expires_in * 1000)
	}
...
const getToken = async (options) => {
	...
	let expiredAt = wepy.getStorageSync('access_token_expired_ar')
